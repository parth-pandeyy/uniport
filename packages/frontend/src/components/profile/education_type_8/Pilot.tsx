import { education_type_8_DataBlock, education_type_8_FormBlock, education_type_8_InputPayload } from "@uniport/common";
...
import { EducationType8ActionDialog } from "./ActionDialog";
import { EducationType8Block } from "./BlockInstance";


const currDate = dayjs().format('YYYY-MM-DD')
// default values (to be used while adding a new block from scratch)
const noValue: education_type_8_FormBlock = {
	block_proof_data: '',
	board: '',
...
export const EducationType8Pilot = ({ meta }) => {
	let {
		attribute_id,
		attribute_type,
		is_array,
	...
	const handleSubmit = async (e: education_type_8_DataBlock) => {
		try {
			setLoading(true);
			setSuccessMessage('');
			setError(null);

			let payload: education_type_8_InputPayload = {
				...e,
				student_id,
				attribute_id,
				attribute_type,
	...
	return (
		<div>
			<div className="bg-white px-4 py-4 flex my-2 rounded-lg shadow flex-col gap-2">
				...
				{open && <EducationType8ActionDialog
