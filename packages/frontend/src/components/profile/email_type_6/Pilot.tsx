import { email_type_6_DataBlock, email_type_6_FormBlock, email_type_6_InputPayload } from "@uniport/common";
...
import { EmailType6ActionDialog } from "./ActionDialog";
import { EmailType6Block } from "./BlockInstance";


// default values (to be used while adding a new block from scratch)
const noValue: email_type_6_FormBlock = {
	block_proof_data: '',
	value: ''
}
...
export const EmailType6Pilot = ({ meta }) => {
	let {
		attribute_id,
		attribute_type,
		is_array,
	...
	const handleSubmit = async (e: email_type_6_DataBlock) => {
		try {
			setLoading(true);
			setSuccessMessage('');
			setError(null);

			let payload: email_type_6_InputPayload = {
				...e,
				student_id,
				attribute_id,
				attribute_type,
