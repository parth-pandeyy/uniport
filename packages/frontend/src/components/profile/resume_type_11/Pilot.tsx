import { resume_type_11_DataBlock, resume_type_11_FormBlock, resume_type_11_InputPayload } from "@uniport/common";
...
import { ResumeActionDialog } from "./ActionDialog";
import { ResumeBlock } from "./BlockInstance";

// Guide
// Change noValue
// Change fieldToPopulate
// Change the type in [handleSubmit]
// see how shall we add the file proof thing

// make [TYPE]Block
// make [TYPE]ActionDialog

// default values (to be used while adding a new block from scratch)
const noValue: resume_type_11_FormBlock = {
	file_name: '',
	resume_file_data: '',
}
...
const fieldToPopulate = ['file_name'];

export const ResumePilot = ({ meta }) => {
	let {
		attribute_id,
		attribute_type,
		is_array,
		...
		is_blocked,
		required,
		// [requires_proof] for resume is always False. (Reason in resolvers)
		requires_proof,
	} = meta;
	...
	const handleSubmit = async (e: resume_type_11_FormBlock) => {
		try {
			setLoading(true);
			setSuccessMessage('');
			setError(null);

			let payload: resume_type_11_InputPayload = {
				...e,
				student_id,
				attribute_id,
				attribute_type,
