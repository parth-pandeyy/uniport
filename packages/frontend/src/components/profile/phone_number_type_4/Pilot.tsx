import { phone_number_type_4_DataBlock, phone_number_type_4_FormBlock, phone_number_type_4_InputPayload } from "@uniport/common";
...
import { PhoneNumberType4ActionDialog } from "./ActionDialog";
import { PhoneNumberType4Block } from "./BlockInstance";


// default values (to be used while adding a new block from scratch)
const noValue: phone_number_type_4_FormBlock = {
	block_proof_data: '',
	country_code: '',
	ph_number: ''
}
...
export const PhoneNumberType4Pilot = ({ meta }) => {
	let {
		attribute_id,
		attribute_type,
		is_array,
	...
	const handleSubmit = async (e: phone_number_type_4_DataBlock) => {
		try {
			setLoading(true);
			setSuccessMessage('');
			setError(null);

			let payload: phone_number_type_4_InputPayload = {
				...e,
				student_id,
				attribute_id,
				attribute_type,
	...
	return (
		<div>
			<div className="bg-white px-4 py-4 flex my-2 rounded-lg shadow flex-col gap-2">
				...
				{open && <PhoneNumberType4ActionDialog
