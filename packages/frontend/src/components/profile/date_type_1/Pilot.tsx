import { date_type_1_DataBlock, date_type_1_FormBlock, date_type_1_InputPayload } from "@uniport/common";
...
import { DateType1ActionDialog } from "./ActionDialog";
import { DateType1Block } from "./BlockInstance";


// default values (to be used while adding a new block from scratch)
const noValue: date_type_1_FormBlock = {
	block_proof_data: '',
	value: '',
}
...
export const DateType1Pilot = ({ meta }) => {
	let {
		attribute_id,
		attribute_type,
		is_array,
	...
	const handleSubmit = async (e: date_type_1_DataBlock) => {
		try {
			setLoading(true);
			setSuccessMessage('');
			setError(null);

			let payload: date_type_1_InputPayload = {
				...e,
				student_id,
				attribute_id,
				attribute_type,
