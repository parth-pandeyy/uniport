import { number_type_2_DataBlock, number_type_2_FormBlock, number_type_2_InputPayload } from "@uniport/common";
...
import { NumberType2ActionDialog } from "./ActionDialog";
import { NumberType2Block } from "./BlockInstance";


// default values (to be used while adding a new block from scratch)
const noValue: number_type_2_FormBlock = {
	block_proof_data: '',
	value: '',
}
...
export const NumberType2Pilot = ({ meta }) => {
	let {
		attribute_id,
		attribute_type,
		is_array,
	...
	const handleSubmit = async (e: number_type_2_DataBlock) => {
		try {
			setLoading(true);
			setSuccessMessage('');
			setError(null);

			let payload: number_type_2_InputPayload = {
				...e,
				student_id,
				attribute_id,
				attribute_type,
