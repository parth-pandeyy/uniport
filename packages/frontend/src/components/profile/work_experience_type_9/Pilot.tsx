import { work_experience_type_9_FormBlock, work_experience_type_9_InputPayload } from "@uniport/common";
...
import { WorkExperienceType9ActionDialog } from "./ActionDialog";
import { WorkExperienceType9Block } from "./BlockInstance";


const currDate = dayjs().format('YYYY-MM-DD')
// default values (to be used while adding a new block from scratch)
const noValue: work_experience_type_9_FormBlock = {
	company_name: '',
	job_title: '',
	location: '',
	position_type: '',
	job_start_date: currDate,
	job_end_date: currDate,
	details: '',
	block_proof_data: ''
}
...
const fieldToPopulate = ['company_name', 'job_title', 'location', 'position_type', 'job_start_date', 'job_end_date', 'details'];


export const WorkExperienceType9Pilot = ({ meta }) => {
	let {
		attribute_id,
		attribute_type,
		is_array,
	...
	const handleSubmit = async (e: work_experience_type_9_FormBlock) => {
		try {
			setLoading(true);
			setSuccessMessage('');
			setError(null);

			let payload: work_experience_type_9_InputPayload = {
				...e,
				student_id,
				attribute_id,
				attribute_type,
	...
	return (
		<div>
			<div className="bg-white px-4 py-4 flex my-2 rounded-lg shadow flex-col gap-2">
				...
				{open && <WorkExperienceType9ActionDialog
